...
def get_all_collection_ids(
    ...
    if collection == "biosample_set" and not force:
        return {
            "warning": f"Collection '{collection}' is likely to contain 15,000+ documents. "
                      f"Fetching all IDs may be slow and memory-intensive. "
                      f"Consider using get_random_collection_subset() instead, or set force=True to proceed.",
            "collection": collection,
