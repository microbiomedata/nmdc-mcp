...
from nmdc_mcp.tools import get_random_biosample_subset, get_random_collection_subset, clean_collection_date
...
class TestGetRandomBiosampleSubset:
    ...
    def test_get_random_biosample_subset_basic(self, mock_sample, mock_randint, mock_fetch):
        # Setup mocks
...
class TestGetRandomCollectionSubset:
    ...
    @patch('nmdc_mcp.tools.random.randint')
    @patch('nmdc_mcp.tools.random.sample')
    def test_get_random_collection_subset_basic(self, mock_sample, mock_randint, mock_fetch):
        # Setup mocks
        mock_randint.return_value = 5
    ...
    def test_get_random_collection_subset_api_error(self, mock_fetch):
        ...
        assert len(result) == 1
        assert "error" in result[0]
        assert "Failed to fetch random samples from study_set" in result[0]["error"]
